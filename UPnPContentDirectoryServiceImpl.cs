...
namespace MediaPortal.Extensions.MediaServer
{
    public class UPnPContentDirectoryServiceImpl : DvService
    {
        ...
        static UPnPError OnBrowse(DvAction action, IList<object> inParams, out IList<object> outParams, CallContext context)
        {
            ...
            Logger.Debug("MediaServer - OnBrowse(objectId=\"{0}\",browseFlag=\"{1}\",filter=\"{2}\",startingIndex=\"{3}\",requestedCount=\"{4}\",sortCriteria=\"{5}\")", objectId, browseFlag, filter, startingIndex, requestedCount, sortCriteria);
            ...
            Logger.Debug("MediaServer - OnBrowse(objectId=\"{0}\"...) = (numberReturned=\"{1}\",totalMatches=\"{2}\",containerUpdateId=\"{3}\") {4}", objectId, numberReturned, totalMatches, containterUpdateId, xml);
